...
- **Genshin Impact** is the first Video Games extension and is opt-in by default. Runtime reads checked-in JSON from `AnimeCharacters/wwwroot/data/extensions/genshin-impact-characters.json` and checked-in artwork from `AnimeCharacters/wwwroot/images/extensions/genshin-impact`. Playable/upcoming characters and voiced NPCs share the extension ID and game media title but render in separate `Genshin Impact` and `Genshin Impact NPC` credit sections.
- Refresh Genshin data with `.\tools\Update-GenshinImpactData.ps1` on Windows or `./tools/update-genshin-impact-data.sh` on macOS/Linux. Both scripts read Fandom's `Character/List` icon data and the `Voice_Actor` plus `Voice_Actor/NPCs` tables, split multi-voice rows into separate character credits, cache available poster/character images locally, and rate-limit API calls; pass `-ResolveAniListIds` or `--resolve-anilist-ids` only when you intentionally want the slower AniList staff ID enrichment.
...
- **Tenrai API** - Primary cached source for MyAnimeList character and Japanese voice actor data; transient failures are retried and `ReferenceApis:TenraiBaseUrl` selects its Jikan-compatible endpoint
- **Jikan API** - Secondary source for MyAnimeList character and Japanese voice actor data; `ReferenceApis:JikanBaseUrl` can select a compatible private/self-hosted endpoint
- **AniList GraphQL API** - Final fallback source for character and voice actor information
