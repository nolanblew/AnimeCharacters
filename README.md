...
Kitsu remains the source for user library data. Character, anime reference, and seiyuu data is loaded through the `ReferenceApis` project so providers can be swapped or combined without changing page logic. Jikan is currently registered first because it exposes character and Japanese voice actor data by MAL anime id, while AniList remains available as a fallback provider. Voice actor routes include the displayed staff name so a failed or stalled staff lookup can resolve an exact name match through another configured provider without changing the primary anime provider. Jikan requests retry transient network, rate-limit, and server failures within a 15-second provider timeout; `ReferenceApis:JikanBaseUrl` in `wwwroot/appsettings.json` can point deployments at a compatible private or self-hosted Jikan instance. AniList fallbacks resolve staff by exact name, fetch only provider IDs and character display data for large histories, and preserve completed history pages if a later page times out; library titles remain the display source.
