...
Kitsu remains the source for user library data. Character, anime reference, and seiyuu data is loaded through the `ReferenceApis` project so providers can be swapped or combined without changing page logic. Tenrai's Jikan-compatible cached API is registered first for character and Japanese voice actor data by MAL anime id, Jikan is the secondary source, and AniList is the final fallback. Voice actor routes include the displayed staff name so a failed or stalled staff lookup can resolve an exact name match through another configured provider without changing the primary anime provider. Tenrai and Jikan requests retry transient network, rate-limit, and server failures within a 15-second provider timeout; `ReferenceApis:TenraiBaseUrl` and `ReferenceApis:JikanBaseUrl` in `wwwroot/appsettings.json` can point deployments at compatible private or self-hosted endpoints. AniList fallbacks resolve staff by exact name, fetch only provider IDs and character display data for large histories, and preserve completed history pages if a later page times out; library titles remain the display source.
